...
// (the page IG/FB traffic gets sent to).
...
// Only the Wild Variety Pack has a dedicated bundle PDP — the copy/images in
// the bundle branch are variety-pack specific, so other bundle ids fall back.
const bundle = slug === 'variety-pack' ? getBundle(slug) : null;
const flavor = bundle ? null : (getFlavor(slug) || FLAVORS[0]);

// Unified "product" the rest of the page renders from
...
// Bundle-specific accordion copy
if (bundle) {
  const inside = document.getElementById('accInside');
  if (inside) inside.textContent = '4 boxes × 8 stick packs = 32 total hydration sticks — one box of each flavor, so every kid in the house gets their favorite critter. Zero sugar, zero artificial dyes, zero junk.';
}
...
document.getElementById('addBtn').addEventListener('click', () => {
  if (bundle) {
    ...
    cartUI?.toast(`${qty}× Wild Variety Bundle added!`);
  } else {
    cart.add(flavor, qty);
...
// ── Other flavors grid ──
document.getElementById('moreGrid').innerHTML = FLAVORS.filter((f) => f.id !== product.id)
  .map(
    (f) => `
